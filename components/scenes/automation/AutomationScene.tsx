...
function CircuitLine({ x1, y1, x2, y2, delay, color = "cyan" }: {
  x1: number; y1: number; x2: number; y2: number; delay: number; color?: "cyan" | "accent";
}) {
...
function PipelineStep({ label, status, icon, delay }: {
  label: string; status: "done" | "active" | "pending"; icon: React.ReactNode; delay: number;
}) {
  return (
    <motion.div
  ...
  return (
    <div className="relative w-full h-[540px] sm:h-[600px] lg:h-[640px]">
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none z-10" viewBox="0 0 700 640">
        ...
        <CircuitLine x1={350} y1={320} x2={130} y2={160} delay={0} />
        <CircuitLine x1={350} y1={320} x2={570} y2={160} delay={0.2} color="accent" />
        <CircuitLine x1={350} y1={320} x2={100} y2={320} delay={0.4} />
        <CircuitLine x1={350} y1={320} x2={600} y2={320} delay={0.6} color="accent" />
        <CircuitLine x1={350} y1={320} x2={130} y2={500} delay={0.8} />
        <CircuitLine x1={350} y1={320} x2={570} y2={500} delay={1.0} color="accent" />
        <CircuitLine x1={350} y1={320} x2={350} y2={100} delay={0.3} />
        <CircuitLine x1={350} y1={320} x2={350} y2={560} delay={0.5} color="accent" />
          ...
          <div className="rounded-xl border border-border-subtle bg-bg-card/90 backdrop-blur-xl p-4 shadow-2xl card-shine">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-1.5">
                <motion.div className="w-1.5 h-1.5 rounded-full bg-cyan" animate={{ scale: [1, 1.4, 1] }} transition={{ duration: 1.2, repeat: Infinity }} />
                <span className="text-[8px] text-cyan font-mono font-semibold">RUNNING</span>
              </div>
            </div>
            <div className="space-y-1.5">
              <PipelineStep label="Réception webhook" status="done" delay={0.9} icon={<svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>} />
              <div className="ml-2.5 w-px h-3 bg-gradient-to-b from-green-400/30 to-cyan/20" />
              <PipelineStep label="Transformation IA" status="done" delay={1.1} icon={<svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>} />
              <div className="ml-2.5 w-px h-3 bg-gradient-to-b from-green-400/30 to-cyan/20" />
              <PipelineStep label="Validation automatique" status="active" delay={1.3} icon={null} />
              <div className="ml-2.5 w-px h-3 bg-gradient-to-b from-cyan/20 to-transparent" />
              <PipelineStep label="Push CRM + Slack" status="pending" delay={1.5} icon={<div className="w-1.5 h-1.5 rounded-full bg-text-tertiary/30" />} />
            </div>
          </div>
        </motion.div>
      </motion.div>
